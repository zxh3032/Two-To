...
func (s *Service) UpdatePassword(ctx *servlet.Context, req *proto.UpdatePasswordRequest, _ *proto.UpdatePasswordResponse) error {
	...
	if err := s.store.UpdatePassword(ctx, ctx.UserID, passwordHash, now); err != nil {
		return internalError(err)
	}
	// 改密后保留当前会话，避免用户在当前设备上被立即踢出。
	if err := s.store.RevokeUserSessions(ctx, ctx.UserID, ctx.SessionID, dao.SessionStatusRevoked, "password_changed", now); err != nil {
		return internalError(err)
	}
	...
	s.logSecurity(ctx, ctx.UserID, "update_password", map[string]string{})
	return nil
}
