...
func (s *Service) PhoneLogin(ctx *servlet.Context, req *proto.PhoneLoginRequest, resp *proto.PhoneLoginResponse) error {
	...
	if identity == nil {
		...
		resp.RequiresProfileSetup = true
	...
	if user == nil || user.Status != dao.UserStatusNormal {
		return apperror.Unauthorized(response.CodeUnauthorized, "账号状态不可用")
	}
	authResp, err := s.issueLogin(ctx, user)
	if err != nil {
		return err
	}
	s.logSecurity(ctx, user.ID, "phone_login", map[string]string{"phone": security.MaskPhone(phone)})
	resp.Auth = authResp
	return nil
}
