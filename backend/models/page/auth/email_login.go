...
func (s *Service) EmailLogin(ctx *servlet.Context, req *proto.EmailLoginRequest, resp *proto.EmailLoginResponse) error {
	...
	if failCount >= s.cfg.Verification.LoginCaptchaFailThreshold {
		if err := s.verifyCaptcha(ctx, req.GetCaptchaId(), req.GetCaptchaCode()); err != nil {
			return apperror.WithData(http.StatusBadRequest, response.CodeBadRequest, "需要完成图片验证码后再登录", map[string]bool{"captchaRequired": true})
		}
	}
